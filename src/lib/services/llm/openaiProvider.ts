...
export function createOpenAIParser(apiKey: string, model = 'gpt-5.4-mini'): ParseTranscriptFn {
	...
	return async (transcript, products) => {
		const response = await openai.responses.create({
			model,
			instructions: `You are a stock counting assistant in a cocktail bar. Extract items and quantities from a voice transcript that has been automatically transcribed and may contain errors.
